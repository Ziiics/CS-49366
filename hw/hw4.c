/******************************************************************************
  ...
  Description : searches through the given directories for all files that
        satisfy the given test. If no directories are given, it uses the
        current working directory
  Purpose : print last login
  Usage : sfind [dir1 dir2 ...] [-s <filename> | -m <fileglob>]
  Build with : gcc -o sfind hw4.c
  Modifications: April 15th, 2024
    ...
    while ((entry = readdir(dp)) != NULL)
    {
        ...
        if (NULL != pattern)
        {
            if (fnmatch(pattern, entry->d_name, 0) == 0)
                ...
                fprintf(stderr, "Can't find matching file(s): %s", pattern);
            exit(EXIT_FAILURE);
        }
...
{
    int ch;
    bool bool_slink = false;   // true if user use -s option
    bool bool_pattern = false; // true if user use -m option
